...
import { Download, RefreshCw, Upload, BarChart3, Radar as RadarIcon, Sparkles, ClipboardCheck, Printer, HelpCircle } from "lucide-react";
import {
  BarChart,
  Bar,
  XAxis,
...
/**
 * 외관 디자인 설문 분석/종합점수 웹앱 (v3.2.2)
 * - 원인: 줄바꿈을 잘못 표기한 문자열로 SyntaxError(unterminated string)
 * - 조치: 모든 줄바꿈을 NL("\n") 상수와 joinNL()로 통일. CSV/텍스트 분기에서도 동일 적용
 * - 추가: 스모크 테스트 보강(줄바꿈/CSV 라인수/textarea 값/임계값 경계)
 */

// ---------- 유틸
const NL = "\n" as const;
const joinNL = (arr: (string|number)[]) => arr.join(NL);
const clamp01 = (x: number) => Math.max(0, Math.min(1, x));
const sum = (arr: number[]) => arr.reduce((a, b) => a + b, 0);
const toPct = (x: number) => `${(x * 100).toFixed(1)}%`;
const nzInt = (v: any) => { const n = parseInt(String(v), 10); return Number.isFinite(n) && n >= 0 ? n : 0; };
const nzFloat = (v: any) => { const n = parseFloat(String(v)); return Number.isFinite(n) ? n : 0; };

// Charts font sizing (reduced)
const CHART_FONT = 9; // px
const tickSmall = { fontSize: CHART_FONT } as const;
const legendSmall = { fontSize: CHART_FONT } as const;
const tooltipSmall = { fontSize: CHART_FONT } as const;
...
function runSmokeTests(){
  try {
    ...
    // 줄바꿈 join/split (핵심 라인: 이전 오류 위치 보강)
    const j = joinNL(['a','b','c']);
    console.assert(j.split(/\r?\n/).length === 3, '줄바꿈 join/split 실패');

    // CSV 줄바꿈
    const csv = joinNL(['H1,H2','1,2','3,4']);
    console.assert(csv.split(/\r?\n/).length === 3, 'CSV 줄바꿈 실패');

    // textarea 값에 줄바꿈 포함 가능
    const tx = joinNL(['라인1','라인2']);
    console.assert(tx.includes('\n') && tx.split('\n').length === 2, 'textarea 줄바꿈 실패');

    // NPS 계산 (총 10표, NPS=20)
    const npsCounts = [1,1,1,0,0,0,0, 1,1, 3,2]; // detr=3, pass=2, prom=5
    ...
    // 변환 함수
    console.assert(nzInt('12') === 12 && nzInt('foo') === 0 && nzInt(-5) === 0, 'nzInt 실패');
    console.assert(nzFloat('1.25') === 1.25 && nzFloat('') === 0, 'nzFloat 실패');

    // 정규화 가중치 합 1
    const w = {Q1:0.5,Q2:0.5};
    const ws = (w.Q1 + w.Q2);
    console.assert(Math.abs(ws-1) < 1e-9, '가중치 합산 실패');

    // 랭킹 로직 간단 체크(경계값 케이스)
    const shares = [[0.7,0.2,0.1,0],[0.6,0.3,0.1,0]]; // #01 우세
    const weights = [0.5,0.5];
    const score0 = shares.reduce((acc,row,i)=>acc+row[0]*weights[i],0);
    const score1 = shares.reduce((acc,row,i)=>acc+row[1]*weights[i],0);
    console.assert(score0 > score1, '랭킹 우선순위 계산 실패');
...
// 공통 소품
const StatChip = ({ label, value, hint }: {label: string; value: React.ReactNode; hint?: string;}) => (
...
const SBlotDefaultWeights: Record<string, number> = { Q1: 0.25, Q2: 0.1, Q3: 0.1, Q4: 0.1, Q5: 0.1, Q6: 0.1, Q7: 0.1, Q8: 0.05, Q9: 0.05, Q10: 0.05 };

function SBlotSection() {
  const [counts, setCounts] = useState<number[][]>(SBlotQuestions.map(() => [0, 0, 0, 0]));
  const [weights, setWeights] = useState<Record<string, number>>({ ...SBlotDefaultWeights });
  const [showGuide, setShowGuide] = useState(false);

  // 저장/복구
  useEffect(() => {
    try {
      const raw = localStorage.getItem("sblot_state_v3_2");
      if (raw) {
        const p = JSON.parse(raw);
        if (Array.isArray(p.counts) && typeof p.weights === 'object') {
          setCounts(p.counts);
  ...
  useEffect(() => {
    try { localStorage.setItem("sblot_state_v3_2", JSON.stringify({ counts, weights })); } catch {}
  }, [counts, weights]);
  ...
  const normalizedWeights = useMemo(() => { const obj: Record<string, number> = {}; SBlotQuestions.forEach((q) => { obj[q] = (weights[q] || 0) / (weightSum || 1); }); return obj; }, [weights, weightSum]);
  ...
  const sblotInterpretation = useMemo(() => {
    ...
    const winner = rankOrder[0]; const runner = rankOrder[1] ?? { score: 0, idx: 0 } as any;
    const band = (s: number) => (s>=80?"매우 강한 선호":s>=70?"강한 선호":s>=60?"보통 이상의 선호":s>=50?"경합 구도":"우세 불충분");
  ...
  const sblotConclusion = useMemo(() => {
    ...
    const winner = rankOrder[0]; const runner = rankOrder[1] ?? { score: 0 } as any; const gap = winner.score - (runner.score ?? 0);
    const flags: string[] = []; if (N < 10) flags.push("표본 수가 작아 불확실성 큼");
    ...
    const lines = [ `결론: ${verdict}`, `근거: 종합점수 ${winner.score.toFixed(1)}점, 2위와 격차 ${gap.toFixed(1)}점`, flags.length?`주의: ${flags.join(' · ')}`:undefined ].filter(Boolean) as string[];
    return joinNL(lines);
  }, [rankOrder, N]);

  const reset = () => { setCounts(SBlotQuestions.map(() => [0,0,0,0])); setWeights({ ...SBlotDefaultWeights }); };

  const exportCountsCSV = () => { const header=["Question","#01","#02","#03","#04","N"]; const rows=counts.map((row,i)=>[SBlotQuestions[i],...row,sum(row)]); const csv=joinNL([header,...rows].map(r=>r.join(','))); const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='sblot_counts.csv'; a.click(); URL.revokeObjectURL(url); };
  const importCountsCSV = async (file: File) => { const text=await file.text(); const lines=text.split(/\r?\n/).map(l=>l.trim()).filter(Boolean); const hasHeader=/question/i.test(lines[0]); const body=hasHeader?lines.slice(1):lines; const next=SBlotQuestions.map(()=>[0,0,0,0]); body.slice(0,10).forEach((line,idx)=>{ const c=line.split(','); next[idx]=[nzInt(c[1]),nzInt(c[2]),nzInt(c[3]),nzInt(c[4])]; }); setCounts(next); };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <Card className="lg:col-span-1 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-foreground/10 shadow-sm">
        <CardContent className="pt-6 space-y-4">
          ...
          <Button variant="outline" size="sm" onClick={()=>setShowGuide(v=>!v)} className="flex items-center gap-2"><HelpCircle className="w-4 h-4"/>{showGuide? '도움말 닫기':'도움말 열기'}</Button>
          {showGuide && (
            <div className="text-xs p-3 rounded-lg bg-muted/60 border">
              {`1) 오른쪽 표에 Q1~Q10 각 문항의 ①~④ 인원수를 넣습니다.${NL}2) 왼쪽 가중치는 그대로 사용해도 됩니다(합계 자동 정규화).${NL}3) 아래 해설문/결론을 복사해 보고서에 붙여 넣으세요.`}
            </div>
          )}
...
const Q1_KEYWORDS = ["깔끔함","의료기기다움","가전제품 같다","장난감 같다","고급스러움","차가움","따뜻함","미니멀","둔탁함","기타"] as const;
const Q6_RING = ["매우 좋다(고급 포인트)","눈에 잘 띄어 좋다","없어도 좋겠다","지저분해 보인다","잘 모르겠다","기타"] as const;
const Q8_ENV = ["욕실 밝은 조명","어두운 화장실","침실 아침","창가/자연광","잘 모르겠다","기타"] as const;
const Q11_SD = ["전문적이다","구식·불편해 보인다","디자인에 무관","잘 모르겠다"] as const;
const Q13_PLACE = ["욕실","침실","화장대","거실","휴대 전용(보관 시 안 보이는 곳)","기타"] as const;
const Q15_COLORS = ["실버","샴페인","미스트 블루","로즈","블랙","기타"] as const;
const Q16_PRICE = ["₩50,000 이하","₩50,000–99,000","₩100,000–199,000","₩200,000 이상"] as const;
...
function ObjSection(){
  const [likertCounts, setLikertCounts] = useState<Record<string, number[]>>(Object.fromEntries(OBJLikertQs.map((q)=>[q.key, [0,0,0,0,0]])));
  const [npsCounts, setNpsCounts] = useState<number[]>(Array.from({length:11}, ()=>0));
  const [catCounts, setCatCounts] = useState<Record<string, number[]>>({ Q1:Array(Q1_KEYWORDS.length).fill(0), Q6:Array(Q6_RING.length).fill(0), Q8:Array(Q8_ENV.length).fill(0), Q11:Array(Q11_SD.length).fill(0), Q13:Array(Q13_PLACE.length).fill(0), Q15:Array(Q15_COLORS.length).fill(0), Q16:Array(Q16_PRICE.length).fill(0) });
  const [showGuide, setShowGuide] = useState(false);

  useEffect(()=>{ try{ const raw=localStorage.getItem('obj_state_v3_2'); if(raw){ const p=JSON.parse(raw); if(p.likertCounts&&p.npsCounts&&p.catCounts){ setLikertCounts(p.likertCounts); setNpsCounts(p.npsCounts); setCatCounts(p.catCounts);} } }catch{} },[]);
  useEffect(()=>{ try{ localStorage.setItem('obj_state_v3_2', JSON.stringify({likertCounts,npsCounts,catCounts})); }catch{} },[likertCounts,npsCounts,catCounts]);

  const likertStats = useMemo(()=>{ const out: Record<string, {N:number;mean:number;top2:number;score100:number}> = {}; OBJLikertQs.forEach(q=>{ const arr=likertCounts[q.key]||[0,0,0,0,0]; const N=sum(arr); const mean=N? arr.map((c,i)=>c*(i+1)).reduce((a,b)=>a+b,0)/N:0; const top2=N? (arr[3]+arr[4])/N:0; const score100=clamp01((mean-1)/4)*100; out[q.key]={N,mean,top2,score100};}); return out; },[likertCounts]);
  ...
  const catTop = (key: string, topK=3)=>{ const arr=catCounts[key]||[]; const labels=(CAT_QUESTIONS.find(q=>q.key===key)||{options:[] as readonly string[]}).options as readonly string[]; return arr.map((c,i)=>({label:labels[i],c})).sort((a,b)=>b.c-a.c).slice(0,topK); };
  const catTotal = (key: string)=> sum(catCounts[key]||[]);
  ...
  const objInterpretation = useMemo(()=>{ const bands=(s:number)=> s>=80?"매우 우수": s>=70?"우수": s>=60?"양호": s>=50?"보통":"개선 필요"; const topLow=[...OBJLikertQs].map(q=>({q:q.key,score:likertStats[q.key].score100})).sort((a,b)=>b.score-a.score); const best=topLow[0]; const worst=topLow[topLow.length-1]; const lines=[ `종합 점수: ${overallScore.toFixed(1)}점 (${bands(overallScore)})`, `강점 문항: ${best?.q??'-'} ( ${(best?.score||0).toFixed(1)}점 )`, `보강 문항: ${worst?.q??'-'} ( ${(worst?.score||0).toFixed(1)}점 )`, `NPS: ${nps.nps.toFixed(1)} — Promoters ${toPct(nps.prom)}, Passives ${toPct(nps.pass)}, Detractors ${toPct(nps.detr)}`, `첫인상 키워드 TOP3: ${catTop('Q1').map(x=>`${x.label}(${x.c})`).join(', ')}`, `대체 컬러 TOP2: ${catTop('Q15',2).map(x=>x.label).join(', ')}` ]; if(ringFavor.tot) lines.push(`링 인상 밸런스(+)${ringFavor.pos} / (±)${ringFavor.neu} / (-)${ringFavor.neg} → 순호감 ${ringFavor.score.toFixed(1)}pt`); if(sdFavor.tot) lines.push(`SD 슬롯 인상 순호감 ${sdFavor.score.toFixed(1)}pt (긍:${sdFavor.pos}, 부:${sdFavor.neg})`); return joinNL(lines); },[overallScore, likertStats, nps, catCounts, ringFavor, sdFavor]);

  const objConclusion = useMemo(()=>{ let verdict='재설계 권장'; if(overallScore>=75 && nps.nps>=30) verdict='출시 적합'; else if (overallScore>=65 && nps.nps>=0) verdict='개선 후 출시 검토'; const priceArr=catCounts.Q16||[]; let priceModeIdx=-1, max=-1; priceArr.forEach((v,i)=>{ if(v>max){max=v; priceModeIdx=i;} }); const priceHint=priceModeIdx>=0? `· 예상 가격대 선호: ${Q16_PRICE[priceModeIdx]}`:''; const lines=[ `결론: ${verdict}`, `근거: 종합 ${overallScore.toFixed(1)} / NPS ${nps.nps.toFixed(1)} · 최약점 ${weakestKey}`, priceHint ].filter(Boolean) as string[]; return joinNL(lines); },[overallScore,nps,weakestKey,catCounts]);

  // --- CSV/JSON Export/Import (OBJ) ---
  // Likert CSV: Question,1,2,3,4,5,N
  const exportLikertCSV = () => {
    const header = ['Question','1','2','3','4','5','N'];
    const rows = OBJLikertQs.map(q=>{ const arr=likertCounts[q.key]||[0,0,0,0,0]; return [q.key, ...arr, sum(arr)]; });
    const csv = joinNL([header,...rows].map(r=>r.join(',')));
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8'});
    const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='obj_likert.csv'; a.click(); URL.revokeObjectURL(url);
  };
  const importLikertCSV = async (file: File) => {
    const text = await file.text();
    const lines = text.split(/\r?\n/).filter(Boolean);
    const body = /question/i.test(lines[0])? lines.slice(1): lines;
    const next = {...likertCounts};
    body.forEach(line=>{ const c=line.split(','); const key=c[0]?.trim(); const nums=[nzInt(c[1]),nzInt(c[2]),nzInt(c[3]),nzInt(c[4]),nzInt(c[5])]; if(next[key]) next[key]=nums; });
    setLikertCounts(next);
  };

  // NPS CSV: Score,Count (0..10)
  const exportNpsCSV = () => {
    const header=['Score','Count'];
    const rows = Array.from({length:11},(_,i)=>[i, npsCounts[i]||0]);
    const csv = joinNL([header,...rows].map(r=>r.join(',')));
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8'});
    const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='obj_nps.csv'; a.click(); URL.revokeObjectURL(url);
  };
  const importNpsCSV = async (file: File) => {
    const text = await file.text();
    const lines = text.split(/\r?\n/).filter(Boolean);
    const body = /score/i.test(lines[0])? lines.slice(1): lines;
    const arr = Array.from({length:11},()=>0);
    body.forEach(line=>{ const parts = line.split(','); const s = parts[0]; const c = parts[1]; const idx = nzInt(s); if(idx>=0 && idx<=10) arr[idx]=nzInt(c); });
    setNpsCounts(arr);
  };

  // Categories CSV (long): Key,Option,Count
  const exportCatsCSV = () => {
    const header=['Key','Option','Count'];
    const rows: (string|number)[][] = [];
    CAT_QUESTIONS.forEach(cq=>{ const labels=cq.options as readonly string[]; const arr=catCounts[cq.key]||[]; labels.forEach((lab,i)=> rows.push([cq.key, lab.replaceAll(',', ' '), arr[i]||0])); });
    const csv=joinNL([header,...rows].map(r=>r.join(',')));
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8'});
    const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='obj_categories.csv'; a.click(); URL.revokeObjectURL(url);
  };
  const importCatsCSV = async (file: File) => {
    const text=await file.text();
    const lines = text.split(/\r?\n/).filter(Boolean);
    const body = /key,?option,?count/i.test(lines[0])? lines.slice(1): lines;
    const next: Record<string, number[]> = {...catCounts};
    body.forEach(line=>{ const parts = line.split(','); const key = parts[0]?.trim(); const count = nzInt(parts[parts.length-1]); const labels=(CAT_QUESTIONS.find(q=>q.key===key)||{options:[] as readonly string[]}).options as readonly string[];
      const maybeIndex = Number(parts[1]);
      const idx = Number.isFinite(maybeIndex) && maybeIndex>=0 ? maybeIndex : labels.findIndex(l=> l.replaceAll(',', ' ') === parts.slice(1, parts.length-1).join(',').trim());
      if(next[key] && idx>=0) { const arr=[...(next[key]||[])]; arr[idx]=count; next[key]=arr; }
    });
    setCatCounts(next);
  };

  const overallTop2 = useMemo(()=>(((OBJLikertQs.map((q) => likertStats[q.key].top2).reduce((a,b)=>a+b,0)||0)/OBJLikertQs.length)*100)||0,[likertStats]);
  const copyText = async (text: string) => { try{ await navigator.clipboard.writeText(text);}catch{} };
  const catBarData = (key: string) => { const labels=(CAT_QUESTIONS.find(x=>x.key===key)||{options:[] as readonly string[]}).options as readonly string[]; const arr=catCounts[key]||[]; return labels.map((lab,i)=>({name:lab, count:arr[i]||0})); };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <Card className="lg:col-span-1 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-foreground/10 shadow-sm">
        <CardContent className="pt-6 space-y-4">
          ...
          {showGuide && (
            <div className="text-xs p-3 rounded-lg bg-muted/60 border">
              {`Likert(1~5): 각 점수별 인원수 입력. NPS(0~10): 점수별 인원수 입력.${NL}카테고리: 보기에 해당하는 응답 수 입력. 좌측 하단 CSV 버튼으로 일괄 불러오기/내보내기 가능.`}
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <Button variant="secondary" onClick={()=>{ setLikertCounts(Object.fromEntries(OBJLikertQs.map((q)=>[q.key,[0,0,0,0,0]]))); setNpsCounts(Array.from({length:11},()=>0)); setCatCounts({ Q1:Array(Q1_KEYWORDS.length).fill(0), Q6:Array(Q6_RING.length).fill(0), Q8:Array(Q8_ENV.length).fill(0), Q11:Array(Q11_SD.length).fill(0), Q13:Array(Q13_PLACE.length).fill(0), Q15:Array(Q15_COLORS.length).fill(0), Q16:Array(Q16_PRICE.length).fill(0) }); }}><RefreshCw className="w-4 h-4 mr-2"/>초기화</Button>

            {/* Likert CSV */}
            <Button onClick={exportLikertCSV}><Download className="w-4 h-4 mr-2"/>Likert CSV</Button>
            <label className="inline-flex items-center gap-2 text-sm cursor-pointer">
              <input type="file" accept=".csv" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) importLikertCSV(f);}}/>
              <span className="inline-flex items-center px-3 py-2 rounded-md border"><Upload className="w-4 h-4 mr-2"/>Likert 불러오기</span>
            </label>

            {/* NPS CSV */}
            <Button onClick={exportNpsCSV}><Download className="w-4 h-4 mr-2"/>NPS CSV</Button>
            <label className="inline-flex items-center gap-2 text-sm cursor-pointer">
              <input type="file" accept=".csv" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) importNpsCSV(f);}}/>
              <span className="inline-flex items-center px-3 py-2 rounded-md border"><Upload className="w-4 h-4 mr-2"/>NPS 불러오기</span>
            </label>

            {/* Categories CSV */}
            <Button onClick={exportCatsCSV}><Download className="w-4 h-4 mr-2"/>카테고리 CSV</Button>
            <label className="inline-flex items-center gap-2 text-sm cursor-pointer">
              <input type="file" accept=".csv" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) importCatsCSV(f);}}/>
              <span className="inline-flex items-center px-3 py-2 rounded-md border"><Upload className="w-4 h-4 mr-2"/>카테고리 불러오기</span>
            </label>
          </div>
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl p-4 bg-gradient-to-br from-muted/70 to-muted/30 border">
              ...
              <div className="text-3xl font-bold">{overallTop2.toFixed(1)}%</div>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={OBJLikertQs.map(q=> ({name:q.key, score: likertStats[q.key].score100}))}>
                <XAxis dataKey="name" tick={tickSmall} />
                <YAxis domain={[0,100]} tick={tickSmall}/>
                <Tooltip wrapperStyle={tooltipSmall}/>
                <Legend wrapperStyle={legendSmall}/>
                  ...
                  <div className="h-56">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={catBarData(ck)}>
                        ...
                        />
                        <YAxis allowDecimals={false} tick={tickSmall}/>
                        <Tooltip wrapperStyle={tooltipSmall}/>
                        <Legend wrapperStyle={legendSmall}/>
        ...
        <header className="rounded-2xl p-6 bg-gradient-to-br from-foreground/5 to-foreground/0 border">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={handlePrint}><Printer className="w-4 h-4 mr-2"/>PDF로 내보내기</Button>
            </div>
          </div>
        </header>
